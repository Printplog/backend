...
# Direct BNB Smart Chain gateway. Keep ``cpay`` as the default until the direct
# gateway has been configured and verified in production. The gateway private
# key must be supplied only as a protected runtime secret.
PAYMENT_GATEWAY_PROVIDER = os.getenv("PAYMENT_GATEWAY_PROVIDER", "cpay").strip().lower()
BSC_RPC_URL = os.getenv("BSC_RPC_URL", "")
BSC_CHAIN_ID = env_int("BSC_CHAIN_ID", 56)
BSC_USDT_CONTRACT_ADDRESS = os.getenv(
    "BSC_USDT_CONTRACT_ADDRESS",
    "0x55d398326f99059fF775485246999027B3197955",
)
BSC_GATEWAY_WALLET_ADDRESS = os.getenv("BSC_GATEWAY_WALLET_ADDRESS", "")
BSC_GATEWAY_PRIVATE_KEY = os.getenv("BSC_GATEWAY_PRIVATE_KEY", "")
BSC_REQUIRED_CONFIRMATIONS = max(1, env_int("BSC_REQUIRED_CONFIRMATIONS", 3))
BSC_RPC_TIMEOUT_SECONDS = env_int("BSC_RPC_TIMEOUT_SECONDS", 15)
BSC_LOG_BLOCK_RANGE = max(1, min(10, env_int("BSC_LOG_BLOCK_RANGE", 10)))
BSC_DEPOSIT_LOOKBACK_BLOCKS = max(0, env_int("BSC_DEPOSIT_LOOKBACK_BLOCKS", 2))
BSC_GAS_LIMIT_MULTIPLIER_PERCENT = max(100, env_int("BSC_GAS_LIMIT_MULTIPLIER_PERCENT", 120))
BSC_LIVE_PAYOUTS_ENABLED = env_bool("BSC_LIVE_PAYOUTS_ENABLED", False)
BSC_LIVE_SWEEPS_ENABLED = env_bool("BSC_LIVE_SWEEPS_ENABLED", False)
BSC_SWEEP_GAS_FUNDING_MULTIPLIER_PERCENT = max(
    100,
    env_int("BSC_SWEEP_GAS_FUNDING_MULTIPLIER_PERCENT", 125),
)
ALCHEMY_WEBHOOK_ID = os.getenv("ALCHEMY_WEBHOOK_ID", "")
ALCHEMY_WEBHOOK_SIGNING_KEY = os.getenv("ALCHEMY_WEBHOOK_SIGNING_KEY", "")
ALCHEMY_NOTIFY_AUTH_TOKEN = os.getenv("ALCHEMY_NOTIFY_AUTH_TOKEN", "")
ALCHEMY_WEBHOOK_TYPE = os.getenv("ALCHEMY_WEBHOOK_TYPE", "graphql").strip().lower()
ALCHEMY_CUSTOM_ADDRESS_VARIABLE = os.getenv(
    "ALCHEMY_CUSTOM_ADDRESS_VARIABLE",
    "sharptoolzDepositRecipients",
).strip()
ALCHEMY_NOTIFY_TIMEOUT_SECONDS = env_int("ALCHEMY_NOTIFY_TIMEOUT_SECONDS", 10)
